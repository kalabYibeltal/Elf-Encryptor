...
int main()
{
    // Create elfio reader for the output elf
    elfio reader;
    elfio writer;

    // Load ELF data
    if (!reader.load("qsort-demo.elf"))
    {
    ...
    for (int i = 0; i < sec_num; ++i)
    {
        ...
        // Go throught the symbol table and collect the addresses of the jumptables and the global copy table and the addi and lui instructions
        if (psec->get_type() == SHT_SYMTAB)
        {
            ...
            for (unsigned int j = 0; j < symbols.get_symbols_num(); ++j)
            {
                ...
                // jump table and global copy table and addi and lui instructions
                if (name.find(".SnowflakeIoT") != std::string::npos)
                {
                    ...
                    // since they come in pairs we need to we need to put them in a map using the function name as the key
                    // luihi instructions
                    else if (name.find("$luihi") != std::string::npos)
                    {
                        ...
                        {
                            // we need the name to match the addi and lui instructions
    ...
    uint16_t k_code_ptr_16 = 0x5464;     // 16-bit key
    uint32_t k_code_ptr_32 = 0x52068860; // 32-bit key
    ...
    // print addi_lui_enc
    {
        ...
        //     {
        //         std::cout << "addilo: " << val << std::endl;
        //     }
        //     for (const auto &val : pair.second[1])
        //     {
        //         std::cout << "luihi: " << val << std::endl;
        //     }
        //     for (const auto &val : pair.second[2])
        //     {
        //         std::cout << "func_addr: " << val << std::endl;
        //     }
        // }
    }

    // encrypt addi and lui instructions
    // since they come in pairs we need to 
    for (const auto &pair : addi_lui)
    {
    ...
    // go through the text section and encrypt the instructions
    // for compressed instructions we need to encrypt only the last 14 bits
    // for 32-bit instructions we need to encrypt the last 30 bits
    // because the first 2 bits are the bits that determine the instruction type
    for (int i = 0; i < size;)
    {
        ...
        if ((first_half & 0x3) == 0x3)
        {
            ...
            if (i + 2 < size)
            {
                ...
                if (addi_lui_enc.find(base_address + i) != addi_lui_enc.end())
                {
                    ...
                    {
                        // uint32_t enc_instruction = enc_instr;
                        // enc_instruction should be the most 30 bits of (enc_instr) ^ k_code_32 ^ (base_address + i)
                        // extract the last 30 bits of enc_instr
                        uint32_t least_2_bits = enc_instr & 0x3;
                        uint32_t most_30_bits = (enc_instr >> 2) ^ k_code_30 ^ ((base_address + i) & 0x3FFFFFFF);

                        uint32_t enc_instruction = (most_30_bits << 2) | least_2_bits;
                ...
                else
                {
                    ...
                    // }
                    uint32_t least_2_bits = instruction & 0x3;
                    uint32_t most_30_bits = (instruction >> 2) ^ k_code_30 ^ ((base_address + i) & 0x3FFFFFFF);
                    uint32_t enc_instruction = (most_30_bits << 2) | least_2_bits;

                    enc_instructions_vec.push_back(std::make_pair(instruction, false));
                    // enc_instructions_vec.push_back(std::make_pair(enc_instruction, false));

                    //  ^ (( base_address + i ) & 0x3FFFFFFF);
        ...
        else
        {
            ...
            uint16_t least_2_bits = first_half & 0x3;
            uint16_t most_14_bits = (first_half >> 2) ^ k_code_14 ^ ((base_address + i) & 0x3FFF);
            uint32_t enc_instruction = (most_14_bits << 2) | least_2_bits;
            //

            // enc_instructions_vec.push_back(std::make_pair(enc_instruction, true));
            enc_instructions_vec.push_back(std::make_pair(first_half, true));
            // std::cout << "encrypted instruction :" ;
            // std::cout << std::hex << std::setw(8) << std::setfill('0') << enc_instruction << std::endl;
    ...
    // // encrypt the .vector section
    std::vector<std::pair<uint32_t, bool>> enc_instructions_vectors_vec;
    section *vector_sec = reader.sections[".vectors"];

    base_address = vector_sec->get_address();
    p = vector_sec->get_data();
    size = vector_sec->get_size();
    ...
    for (int i = 0; i < size;)
    {
        ...
        if ((first_half & 0x3) == 0x3)
        {
            ...
            if (i + 2 < size)
            {
                ...
                enc_instructions_vectors_vec.push_back(std::make_pair(enc_instruction, false));
        ...
        else
        {
            ...
            uint16_t most_14_bits = (first_half >> 2) ^ k_code_14 ^ ((base_address + i) & 0x3FFF);
            uint32_t enc_instruction = (most_14_bits << 2) | least_2_bits;
            enc_instructions_vectors_vec.push_back(std::make_pair(enc_instruction, true));
            i += 2;
        }
    }
    ...
    if (psec->get_name() == ".rodata")
    {
        ...
        for (int i = 0; i < size;)
        {
            ...
            else if (jumpt_adds_index > 1 && (base_address + i) <<)
            {
    ...
    for (int i = 0; i < sec_num; ++i)
    {
        ...
        // else if ( sec->get_name() == ".vectors" ) {

        //     for (const auto& instr_data : enc_instructions_vectors_vec) {
        //         const char* data = reinterpret_cast<const char*>(&instr_data.first);
        //         if (instr_data.second) {
        //             new_sec->append_data(data, 2);
        //         } else {
    ...
    // no need to tamper with this code
    for (int i = 0; i < seg_num; ++i)
    {
